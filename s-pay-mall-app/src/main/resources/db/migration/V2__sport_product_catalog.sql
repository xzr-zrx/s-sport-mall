ALTER TABLE product
    ADD COLUMN category VARCHAR(32) NOT NULL DEFAULT 'MMA' AFTER name;

CREATE INDEX idx_product_category_status ON product(category, status);

UPDATE product SET category = 'MMA' WHERE category IS NULL OR category = '';
...
INSERT INTO product(name, category, description, cover_url, price, stock, status)
SELECT '高弹支撑运动护膝', 'PROTECTION', '分区编织与侧向支撑条设计，为深蹲、弓步和 grappling 提供保护。', '/images/products/knee-support.svg', 79.00, 155, 'ON_SALE'
